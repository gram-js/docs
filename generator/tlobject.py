...
def snake_to_camel_case(name, upper=True):
    result = re.sub(r'_([a-z])', lambda m: m.group(1).upper(), name)
    result = (result[:1].upper() if upper else result[:1].lower()) + result[1:].replace('_', '')
    return result
...
class TLObject:
    def __init__(self, fullname, object_id, args, result,
        ...
        if object_id is None:
            ...
            whitelist = WHITELISTED_MISMATCHING_IDS[0] | WHITELISTED_MISMATCHING_IDS.get(layer, set())

        self.class_name = snake_to_camel_case(
            self.name)

        self.real_args = list(a for a in self.sorted_args() if not (a.flag_indicator or a.generic_definition))
