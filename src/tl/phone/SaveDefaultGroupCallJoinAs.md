...
(async function run() {
    ...
    const result = await client.invoke(new Api.phone.SaveDefaultGroupCallJoinAs({
    peer: 'username',
    joinAs: 'username'
}));
...
(async function run() {
    ...
    const result: Api.Bool = await client.invoke(new Api.phone.SaveDefaultGroupCallJoinAs({
    peer: 'username',
    joinAs: 'username'
}));
    console.log(result); // prints the result
