...
(async function run() {
    const result = await client.invoke(new Api.messages.ReadMessageContents({
    id: [43]
}));
...
(async function run() {
    const result: Api.messages.AffectedMessages = await client.invoke(new Api.messages.ReadMessageContents({
    id: [43]
}));
    console.log(result); // prints the result
})();
