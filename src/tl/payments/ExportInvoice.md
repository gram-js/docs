...
(async function run() {
    ...
    const result = await client.invoke(new Api.payments.ExportInvoice({
...
(async function run() {
    ...
    const result: Api.payments.ExportedInvoice = await client.invoke(new Api.payments.ExportInvoice({
    invoiceMedia: new Api.InputMediaUploadedPhoto({
        file: await client.uploadFile({file:new CustomFile("file.bin", fs.statSync("../file.bin").size, "../file.bin"),workers:1}),
        stickers: [new Api.InputDocument({
            id: BigInt('-4156887774564'),
