...
(async function run() {
    ...
    const result = await client.invoke(new Api.payments.RequestRecurringPayment({
    userId: 'username',
...
(async function run() {
    ...
    const result: Api.Updates = await client.invoke(new Api.payments.RequestRecurringPayment({
    userId: 'username',
    recurringInitCharge: 'some string here',
    invoiceMedia: new Api.InputMediaUploadedPhoto({
        file: await client.uploadFile({file:new CustomFile("file.bin", fs.statSync("../file.bin").size, "../file.bin"),workers:1}),
