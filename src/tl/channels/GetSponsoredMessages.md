...
(async function run() {
    ...
    const result = await client.invoke(new Api.channels.GetSponsoredMessages({
    channel: 'username'
}));
...
(async function run() {
    ...
    const result: Api.messages.SponsoredMessages = await client.invoke(new Api.channels.GetSponsoredMessages({
    channel: 'username'
}));
    console.log(result); // prints the result
})();
